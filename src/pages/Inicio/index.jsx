  ...
  const loadData = async () => {
    ...
    try {
      ...
      const promessas = categoriasArray.map(async (cat) => {
        ...
        return {
          filmes: fSnap.docs.map(d => ({ id: d.id, tipo: 'filme', categoriaId: cat.id, ...d.data() })),
          series: sSnap.docs.map(d => ({ id: d.id, tipo: 'serie', categoriaId: cat.id, ...d.data() }))
        };
      });
  ...
  return (
    <div className="min-h-screen bg-[#070707] text-white font-sans selection:bg-red-600 overflow-x-hidden">
        ...
        <section className="relative w-full h-[85vh] flex items-end pb-24 px-6 md:px-16 overflow-hidden">
          ...
          >
            <div className="flex items-center gap-3 mb-4">
                ...
                <span className="text-zinc-400 text-xs font-bold uppercase tracking-widest underline decoration-red-600">Destaque</span>
            </div>
                ...
                {itensDaCategoria.map(item => (
                  <motion.div 
                    ...
                    onClick={() => {
                        ...
                        const isSerie = item.tipo === 'serie' || item.temporadas || item.capitulos;
                        router.push(isSerie ? `/idseries?id=${item.id}` : `/idfilmes?id=${item.id}`);
                    }}
                  >
      ...
      >
        ...
        <span className="text-[11px] font-black uppercase tracking-widest hidden sm:block">SkyChat</span>
      </motion.button>
      ...
      <style jsx global>{`
        ...
        body { background-color: #070707; margin: 0; }
      `}</style>
    </div>
  );
}   